...
import kotlinx.android.synthetic.main.number_input.home

class HardTAGameActivity : AppCompatActivity() {
    var cnt = 0
    val hnd0= Handler()
    ...
    private final var TAG = "HardTAGameActivity"
    ...
    fun createQuestion(){
        ...
        val numnber_c = (1..20).random()
        var symbol_a = (0..1).random()
        var symbol_b = (0..1).random()
        var symbol_c = (0..1).random()
        ...
        if(symbol_c == 0){
            x1_number.text = "+$numnber_c"
        }else{
            x1_number.text = "-$numnber_c"
        }
        answerFirstNumber = numnber_a * symbol_a * 2
        answerSecondNumber = numnber_b * symbol_b

        val questionNumbgerA = numnber_a * symbol_a
        val questionNumbgerB = answerSecondNumber
...
//        }else
    ...
    fun saveTime(){
        ...
        val saveInt = pref.getInt("ta_time_hard", 9999)
        var newInt = saveInt
    ...
    fun clickEvent(){
        ...
        enter.setOnClickListener {
            if(answer_2_side.getText().toString() != "" && answer_1.getText().toString() != "" && answer_2.getText().toString() != ""){
                checkTheAnswer()
            }else{
                batu.visibility = View.VISIBLE
                Handler(Looper.getMainLooper()).postDelayed({
    ...
    fun videoAds(){
        ...
        adView_ta.loadAd(adRequest)
        InterstitialAd.load(this,"ca-app-pub-5047266594533407/9240835669", adRequest, object : InterstitialAdLoadCallback() {
